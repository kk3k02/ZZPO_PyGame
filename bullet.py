...
class Bullet(object):
    def __init__(self, sw, sh, player):
        ...
        self.point = player.head  # Bullet starting point (same as player's head position)
        self.x, self.y = self.point  # Current position of the bullet
        self.w = 20  # Width of the bullet
        self.h = 20  # Height of the bullet
        self.c = player.cosine  # Cosine of the player's angle (used for bullet's horizontal velocity)
        self.s = player.sine  # Sine of the player's angle (used for bullet's vertical velocity)
    ...
    def draw(self, win):
        ...
        pygame.draw.circle(win, (255, 255, 255), (self.x + self.w // 2, self.y + self.h // 2), self.w // 2)
