...
class Game:
    def __init__(self):
        ...
        self.bangLargeSound.set_volume(.25)
        self.bangSmallSound.set_volume(.25)
        self.backgroundSound.set_volume(.20)
    ...
    def redrawGameWindow(self):
        ...
        if self.rapidFire:
            ...
            pygame.draw.rect(self.win, (255, 255, 255),
                             [self.sw // 2 - 50, 20, 100 - 100 * (self.count - self.rfStart) / 500, 20])

        if self.gameover:
            self.win.blit(playAgainText, (self.sw // 2 - playAgainText.get_width() // 2,
        ...
        self.win.blit(highScoreText, (self.sw - highScoreText.get_width() - 25, 35 + scoreText.get_height()))
        ...
        while run:
            ...
            if not self.gameover:
                ...
                if self.count % 50 == 0:
                    ...
                    self.asteroids.append(Virus(self.sw, self.sh, ran, self.asteroids_pics))
                ...
                for i, a in enumerate(self.aliens):
                    a.x += a.xv
                    a.y += a.yv
                    if a.x > self.sw + 150 or a.x + a.w < -100 or a.y > self.sh + 150 or a.y + a.h < -100:
                        self.aliens.pop(i)
                    if self.count % 60 == 0:
                        self.alienBullets.append(EnemyBullet(self.player, a.x + a.w // 2, a.y + a.h // 2))
                ...
                for i, b in enumerate(self.alienBullets):
                    b.x += b.xv
                    b.y += b.yv
                    if ((self.player.x - self.player.w // 2 <= b.x <= self.player.x + self.player.w // 2) or
                            self.player.x - self.player.w // 2 <= b.x + b.w <= self.player.x + self.player.w // 2):
                        if ((self.player.y - self.player.h // 2 <= b.y <= self.player.y + self.player.h // 2) or
                                self.player.y - self.player.h // 2 <= b.y + b.h <= self.player.y + self.player.h // 2):
                            self.lives -= 1
                ...
                for a in self.asteroids:
                    ...
                    if ((self.player.x - self.player.w // 2 <= a.x <= self.player.x + self.player.w // 2) or
                            (self.player.x + self.player.w // 2 >= a.x + a.w >= self.player.x - self.player.w // 2)):
                        if ((self.player.y - self.player.h // 2 <= a.y <= self.player.y + self.player.h // 2) or
                                (self.player.y - self.player.h // 2 <= a.y + a.h <= self.player.y + self.player.h // 2)):
                            self.lives -= 1
                            ...
                            if (a.y <= b.y <= a.y + a.h) or a.y <= b.y + b.h <= a.y + a.h:
                                if a.rank == 3:
                                    if self.isSoundOn:
                                        self.bangLargeSound.play()
                                    self.score += 10
                                    na1 = Virus(self.sw, self.sh, 2, self.asteroids_pics)
                                    na2 = Virus(self.sw, self.sh, 2, self.asteroids_pics)
                                    na1.x = a.x
                                    na2.x = a.x
                                    na1.y = a.y
                                    na2.y = a.y
                                    ...
                                    self.score += 20
                                    na1 = Virus(self.sw, self.sh, 1, self.asteroids_pics)
                                    na2 = Virus(self.sw, self.sh, 1, self.asteroids_pics)
                                    na1.x = a.x
                                    na2.x = a.x
                                    na1.y = a.y
                                    na2.y = a.y
                ...
                for s in self.stars:
                    s.x += s.xv
                    s.y += s.yv
                    if s.x < -100 - s.w or s.x > self.sw + 100 or s.y > self.sh + 100 or s.y < -100 - s.h:
